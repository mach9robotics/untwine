...
public:
    struct Chunk
    {
        ...
        IndexedStats stats;
        size_t count;
        // Late materialization: row id of each view point (ids[i] belongs to view point i).
        // Empty in standard mode, where the view already holds all dims.
...
private:
    ...
    const BaseInfo& m_b;
    ThreadPool m_pool;
    // Late materialization: read-only fd of the attribute store. pread doesn't touch the file
    // offset, so one fd is shared by all worker threads.
    int m_attrFd {-1};
};
