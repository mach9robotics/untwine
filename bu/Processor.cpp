    ...
    const FileInfo& fi, IndexIter begin, IndexIter end, std::vector<uint64_t>& ids)
{
    ...
    // Late materialization: the record is just {xyz, rowId} - the per-dim loop below would
    // read past it into the next point. Set xyz, remember the id; the chunk writer gathers
    // the remaining dims from the attribute store.
