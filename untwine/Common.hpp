...
struct Options
{
    ...
    bool chunker;
    // Late materialization (UNTWINE_LATE_MAT=1, --chunker only): slim {xyz, rowId} .bin records
    // plus a single attribute store; the chunk writer gathers the remaining dims at output time.
