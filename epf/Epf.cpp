...
static_assert(MaxBuffers > NumFileProcessors,
              "MaxBuffers must be greater than NumFileProcessors.");
...
void Epf::run(ProgressWriter &progress) {
  ...
  // Setup grid and point count. For each file info the N x N x N grid is
  // expanded to hold all the points. If the number of points seems too large, N
  // is expanded to N + 1. The correct N is often wrong, especially for some
  // areas where things are more dense.
  ...
  // This is just a debug thing that will allow the number of input files to be
  // limited.
  ...
  for (FileInfo &fi : fileInfos) {
    for (FileDimInfo &di : fi.dimInfo) {
      // If this dimension is one of the bit dimensions, set the shift and
      // offset accordingly.
      int bitPos = getUntwineBitPos(di.name);
      if (bitPos != -1) {
        di.type = UntwineBitsType;
        di.offset =
            outputLayout.dimOffset(outputLayout.findDim(UntwineBitsDimName));
        di.shift = bitPos;
      } else {
  ...
  m_writer.reset(
      new Writer(m_b.opts.tempDir, NumWriters, outputLayout.pointSize()));

  // Sort file infos so the largest files come first. This helps to make sure we
  // don't delay processing big files that take the longest (use threads more
  // efficiently).
  std::sort(fileInfos.begin(), fileInfos.end(),
            [](const FileInfo &f1, const FileInfo &f2) {
  ...
  m_pool.join();
  // Tell the writer that it can exit. stop() will block until the writer
  // threads are finished.  stop() will throw if an error occurred during
  // writing.
  m_writer->stop();
  ...
  // Get totals from the current writer that are greater than the
  // MaxPointsPerNode. Each of these voxels that is too large will be
  // reprocessed.
  // ABELL - would be nice to avoid this copy, but it probably doesn't matter
  // much.
  ...
  // Make a new writer since we stopped the old one. Could restart, but why
  // bother with extra code...
...
void Epf::fillMetadata(const pdal::PointLayout &layout) {
  ...
  auto calcScale = [](double scale, double low, double high) {
    ...
    // 2 billion is a little less than the int limit.  We center the data around
    // 0 with the offset, so we're applying the scale to half the range of the
    // data.
  ...
  m_b.scale[0] =
      calcScale(m_b.scale[0], m_b.trueBounds.minx, m_b.trueBounds.maxx);
  m_b.scale[1] =
      calcScale(m_b.scale[1], m_b.trueBounds.miny, m_b.trueBounds.maxy);
  m_b.scale[2] =
      calcScale(m_b.scale[2], m_b.trueBounds.minz, m_b.trueBounds.maxz);
  ...
  auto calcOffset = [](double minval, double maxval, double scale) {
    ...
    double spacings =
        interval / scale; // Number of quantized values in our range.
    double halfspacings = spacings / 2; // Half of that number.
    double offset =
        (int32_t)halfspacings * scale; // Round to an int value and scale down.
    return std::round(
        minval + offset); // Add the base (min) value and round to an integer.
  };

  // Preserve offsets if we have them and --single_file with single input is
  // used
  if (!m_b.preserveHeaderFields() || std::isnan(m_b.offset[0]) ||
      std::isnan(m_b.offset[1]) || std::isnan(m_b.offset[2])) {
    m_b.offset[0] =
        calcOffset(m_b.trueBounds.minx, m_b.trueBounds.maxx, m_b.scale[0]);
    m_b.offset[1] =
        calcOffset(m_b.trueBounds.miny, m_b.trueBounds.maxy, m_b.scale[1]);
    m_b.offset[2] =
        calcOffset(m_b.trueBounds.minz, m_b.trueBounds.maxz, m_b.scale[2]);
  }
}

void Epf::createFileInfos(const StringList &input,
                          std::vector<FileInfo> &fileInfos) {
  using namespace pdal;

  StringList filenames;

  // If any of the specified input files is a directory, get the names of the
  // files in the directory and add them.
  ...
  // Determine a driver for each file and get a preview of the file.  If we
  // couldn't Create a FileInfo object containing the file bounds, dimensions,
  // filename and associated driver.  Expand our grid by the bounds and file
  // point count.
  for (std::string &filename : filenames) {
    ...
    if (driver == "readers.las") {
      const std::vector<FileInfo> &infos =
          processLas(*dynamic_cast<LasReader *>(s), fi);
...
void Epf::filterDims(std::vector<FileInfo> &infos, StringList allowedDims) {
  ...
  // If there are some dim names specified, make sure they contain X, Y and Z
  // and that they're all uppercase.
  ...
  for (FileInfo &info : infos) {
    ...
    auto it = info.dimInfo.begin();
    while (it != info.dimInfo.end()) {
      FileDimInfo &fdi = *it;
      if (!pdal::Utils::contains(allowedDims, pdal::Utils::toupper(fdi.name))) {
...
void Epf::determineDims(std::vector<FileInfo> &infos,
                        pdal::PointLayout &layout) {
  using namespace pdal;

  StringList untypedDimNames;

  // Register dimensions in the OUTPUT layout, transforming dimensions that are
  // packed into the "untwine bits" dimension.
  for (FileInfo &info : infos)
  ...
  // match those that are already in the output layout.
...
std::vector<FileInfo> Epf::processLas(pdal::LasReader &r, FileInfo fi) {
  ...
  // If we have LAS start capability, break apart file info into chunks of size
  // 5 million.
